...
| 2c | **ALEX v02 redesign — implement the fix** | **In progress, 2026-09-11.** Full architectural fix (not just reorder-in-place): `docs/ALEX_v02_redesign_spec.md`. Phases 0 (recon), 1 (Rules-tab write tooling), and 2 (prune, 19 dead dialogs deleted) all done. Phase 3 (spirometry redesign): all 8 new variables created, and the `$currentDaySlot` rule is built and verified (4 gate+assign rule pairs under PERIODIC BASIS). Next: the DAILY/PERIODIC BASIS reset/firing rule rewrite for spirometry itself (rule-level not-answered timeout can't currently be set via automation — a real platform limitation found in Phase 1, not just a gap in the tooling). New finding: the Rules tree's `New` toolbar button unreliably nests children (use `Duplicate`+edit for new siblings instead) — see `autochanges/2026-09-11-alex-v02-phase3-dayslot-rule.md`. | `alex-pileup-analysis` |
